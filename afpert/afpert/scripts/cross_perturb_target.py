...
from afpert.perturb import perturb_disjoint
...
def generate_runs(base_a3m: Path, target: str, query_masks: Iterable[int], msa_masks: Iterable[int], depths: Iterable[int], seed: int, runs_root: Path, overwrite: bool = False) -> List[Path]:
    ...
    for q, m in product(query_masks, msa_masks):
        ...
        if a3m_path.exists() and not overwrite:
            ...
            perturbed = perturb_disjoint(a3m_entries, q / 100.0, m / 100.0, seed)
